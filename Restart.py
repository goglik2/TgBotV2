...
def checkRasp(message):
    try:
        ...
        cur.execute(""f"SELECT group_id FROM classes WHERE id = (SELECT id FROM users WHERE id = ?) = {user_id}""")
        ...
        for item in data[0]:
            for lesson in item:
                messageR += lesson["time"] + ' | ' + lesson["discipline"] + ' | ' + lesson["teacher"] + ' | ' + lesson["place"] + '\n' + '-' + '\n'
        for user in users:
            try:
                infu = f'{user[0]}'
...
def clasrasp(call):
    ...
    if call.data == 'Сегодня':
        try:
            ...
            cur = conn.cursor()
            user_id = call.from_user.id
            cur.execute("""SELECT class_id FROM classes WHERE users.class_name_temp = classes.class_name""")
            selectGroup = cur.fetchall()
            selectGroup = str(selectGroup)
            selectGroup = selectGroup.replace("[", "").replace("]", "").replace("(", "").replace(")", "").replace(",", "").replace("'", "").replace("'", "")
            conn.commit()
